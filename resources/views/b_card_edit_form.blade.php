...
@section("content")
    <?PHP
    foreach ($data as $result_p){
        $folder = $result_p->card_folder;
        $cover = $result_p->card_cover;
    ?>
    <div class="card" >
        ...
        <div class="card-body">
            ...
            <div class="tab-content card" style="z-index: auto;">
                <div class="md-form"  >
                    <input type="hidden"  value="{{ $result_p->card_folder }}"  class="folder">
                    <input type="hidden"  value="{{ $result_p->card_code }}" class="code">
                    <input type="hidden"  value="{{ $result_p->card_cover }}" class="cover">

                    <input type="text" id="title" value="{{ $result_p->card_title }}" class="title_ip">
                ...
                <div class="md-form">
                    ...
                    <textarea type="text" id="editor1"  class="md-textarea form-control detail_ip rounded-0"  rows="20" style="height: 100px;" >{{ $result_p->card_description }}</textarea>
                ...
                <div class="md-form">
                    <select class="mdb-select product">
                        ...
                        <option value="{{ $result->type_code }}" <?PHP if($result_p->card_product == $result->type_code ){ echo "selected"; } ?> >{{ $result->type_title }}</option>
                        <?PHP
                        }
                        ?>
                ...
                <div class="md-form">
                    <select class="mdb-select dimensions">
                        ...
                        <option value="{{ $result->type_code }}" <?PHP if($result_p->card_dimension == $result->type_code ){ echo "selected"; } ?> >{{ $result->type_title }}</option>
                        <?PHP
                        }
                        ?>
                    </select>
                ...
                <div class="md-form">
                    <select class="mdb-select type_paper">
                        ...
                        <option value="{{ $result->type_code }}" <?PHP if($result_p->card_type == $result->type_code ){ echo "selected"; } ?> >{{ $result->type_title }}</option>
                        <?PHP
                        }
                        ?>
                    </select>
                ...
                <div class="md-form">
                    <select class="mdb-select printime">
                        ...
                        <option value="{{ $result->type_code }}" <?PHP if($result_p->card_printing_time == $result->type_code ){ echo "selected"; } ?> >{{ $result->type_title }}</option>
                        <?PHP
                        }
                        ?>
                    </select>
                    ...
                    <div class="md-form">
                        <input type="text" value="{{ $result_p->card_price }}" id="price"  class="price_ip">
        ...
        <div class="card-body">
            <div class="tab-content card">
                ...
                <div class="row" align="center" >
                    <button id="submit_form" class="btn btn-primary ladda-button ladda-progress" data-style="zoom-in" type="button" >{{ trans("tool.update") }}</button>
                </div>
            </div>
...
<script>
   ...
   // Loadgroup("codsx");
    ...
    function cal_input() {
        ...
        form_data.append("cover_name",cover_name);
    ...
    $("#Gallery").fileinput({
        ...
        initialPreviewDownloadUrl:"{{ URL::to('/') }}/Picture/Place/{filename}_Original.jpg",
        initialPreviewConfig: [
                <?PHP

                foreach ($gallery as $resultgallery){
                ?>
            {
                caption: "{{ $resultgallery->gal_o_name }}",
                width: "120px",
                url: "{{ URL::to('/')."/DeleteImageCard" }}",
                key: '{{ $resultgallery->gal_code }}' ,
                filename:'{{ $resultgallery->gal_folder }}/Gallery/{{ $resultgallery->gal_name }}',
                extra: {folder:'{{ $resultgallery->gal_folder }}',
                    namefile:'{{ $resultgallery->gal_name }}'}
    ...
    $("#CoverMain").fileinput({
        ...
        initialPreview: [url+"/Media/Card/{{ $folder }}/{{ $cover }}_Original.jpg"],
        initialPreviewConfig: [
            {caption: "Cover", downloadUrl: url+"/Media/Card/{{ $folder }}/{{ $cover }}_Original.jpg",width: "500px", key: 1},
        ],
